...
<template>
  <div class="flex flex-wrap items-center gap-2">
    <div class="relative min-w-52 flex-1 sm:max-w-xs">
      <Search :size="16" class="pointer-events-none absolute start-3 top-1/2 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
      <input
        type="search"
        :value="search"
        :placeholder="t('tools.entityManager.browse.searchPlaceholder')"
        :aria-label="t('tools.entityManager.browse.searchPlaceholder')"
        class="h-9 w-full rounded-lg border border-border bg-card ps-9 pe-3 text-sm text-foreground transition-shadow placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
    ...
    <div v-if="!isInline" class="inline-flex gap-0.5 rounded-lg bg-secondary p-0.5" role="group">
      <button
        type="button"
        class="h-8 rounded-md px-3 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    ...
    <div class="ms-auto flex items-center gap-2">
      <template v-if="hasSelection">
        ...
        <button
          v-if="canMerge"
          type="button"
          class="inline-flex h-8 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        ...
        <button
          type="button"
          class="inline-flex h-8 items-center gap-1.5 rounded-md border border-destructive/30 px-3 text-xs font-semibold text-destructive transition-colors hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      ...
      <div v-else class="inline-flex gap-0.5 rounded-lg bg-secondary p-0.5" role="group" :aria-label="t('tools.entityManager.browse.density.label')">
        <button
          type="button"
          class="grid size-8 place-items-center rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
