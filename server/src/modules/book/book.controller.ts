...
export class BookController {
  ...
  async downloadFile(
    ...
    try {
      ...
      this.logger.log(
        `[${event}] [end] fileId=${fileId} userId=${user.id} durationMs=${Date.now() - startedAt} status=${result.status} sizeBytes=${size} partial=${result.partial} - download file completed`,
      );
    } catch (err) {
      const errorClass = err instanceof Error ? err.name : 'Error';
      const errorMessage = sanitizeLogValue(err instanceof Error ? err.message : String(err));
